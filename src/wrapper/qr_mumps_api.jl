for (fname, lname, elty, subty) in (
  ("sqrm_spmat_init_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    ...
    function qrm_spmat_init!(
      spmat::qrm_spmat{$elty},
      A::SparseMatrixCSC{$elty, I};
      sym::Bool = false,
    ) where {I <: Integer}
      err = ccall(($fname, $lname), Cint, (Ref{c_spmat{$elty}},), spmat)
      (err ≠ 0) && throw(ErrorException(error_handling(err)))
    ...
    @inline qrm_spmat_init!(
      spmat::qrm_spmat{$elty},
      A::Symmetric{$elty, SparseMatrixCSC{$elty, I}},
    ) where {I <: Integer} = qrm_spmat_init!(spmat, A.data, sym = true)
    @inline qrm_spmat_init!(
      spmat::qrm_spmat{$elty},
      A::Hermitian{$elty, SparseMatrixCSC{$elty, I}},
    ) where {I <: Integer} = qrm_spmat_init!(spmat, A.data, sym = true)

    @inline qrm_spmat_init(A::Symmetric{$elty, SparseMatrixCSC{$elty, I}}) where {I <: Integer} =
      qrm_spmat_init(A.data, sym = true)
    @inline qrm_spmat_init(A::Hermitian{$elty, SparseMatrixCSC{$elty, I}}) where {I <: Integer} =
      qrm_spmat_init(A.data, sym = true)
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_spmat_destroy_c", libsqrm, Float32, Float32),
...
for (fname, lname, elty, subty) in (
  ("sqrm_spfct_init_c", libsqrm, Float32, Float32),
...
for (fname, lname, elty, subty) in (
  ("sqrm_spfct_destroy_c", libsqrm, Float32, Float32),
...
for (fname, lname, elty, subty) in (
  ("sqrm_analyse_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_analyse!(spmat::qrm_spmat{$elty}, spfct::qrm_spfct{$elty}; transp::Char = 'n')
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ref{c_spfct{$elty}}, UInt8),
        spmat,
        spfct,
        transp,
      )
    ...
    @inline qrm_analyse!(spmat::Transpose{$elty, qrm_spmat{$elty}}, spfct::qrm_spfct{$elty}) =
      qrm_analyse!(spmat.parent, spfct, transp = 't')
    @inline qrm_analyse!(spmat::Adjoint{$elty, qrm_spmat{$elty}}, spfct::qrm_spfct{$elty}) =
      qrm_analyse!(spmat.parent, spfct, transp = 'c')

    @inline qrm_analyse(spmat::Transpose{$elty, qrm_spmat{$elty}}) =
      qrm_analyse(spmat.parent, transp = 't')
    @inline qrm_analyse(spmat::Adjoint{$elty, qrm_spmat{$elty}}) =
      qrm_analyse(spmat.parent, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_factorize_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    ...
    @inline qrm_factorize!(spmat::Transpose{$elty, qrm_spmat{$elty}}, spfct::qrm_spfct{$elty}) =
      qrm_factorize!(spmat.parent, spfct, transp = 't')
    @inline qrm_factorize!(spmat::Adjoint{$elty, qrm_spmat{$elty}}, spfct::qrm_spfct{$elty}) =
      qrm_factorize!(spmat.parent, spfct, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_solve_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_solve!(
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spfct{$elty}}, UInt8, Ptr{$elty}, Ptr{$elty}, Cint),
        spfct,
        transp,
        b,
        x,
    ...
    @inline qrm_solve!(
      spfct::Transpose{$elty, qrm_spfct{$elty}},
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_solve!(spfct.parent, b, x, transp = 't')
    @inline qrm_solve!(
      spfct::Transpose{$elty, qrm_spfct{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_solve(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_solve(spfct.parent, b, transp = 't')
    @inline qrm_solve(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_solve(spfct.parent, b, transp = 't')

    @inline qrm_solve!(
      spfct::Adjoint{$elty, qrm_spfct{$elty}},
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_solve!(spfct.parent, b, x, transp = 'c')
    @inline qrm_solve!(
      spfct::Adjoint{$elty, qrm_spfct{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_solve(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_solve(spfct.parent, b, transp = 'c')
    @inline qrm_solve(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_solve(spfct.parent, b, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_apply_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_apply!(spfct::qrm_spfct{$elty}, b::Vector{$elty}; transp::Char = 'n')
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spfct{$elty}}, UInt8, Ptr{$elty}, Cint),
        spfct,
        transp,
        b,
        nrhs,
    ...
    @inline qrm_apply!(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_apply!(spfct.parent, b, transp = 't')
    @inline qrm_apply!(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_apply!(spfct.parent, b, transp = 't')
    ...
    @inline qrm_apply!(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_apply!(spfct.parent, b, transp = 'c')
    @inline qrm_apply!(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_apply!(spfct.parent, b, transp = 'c')
...
for (fname, lname, elty, subty) in (
  ("sqrm_spmat_mv_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_spmat_mv!(
      ...
      err = ccall(
        ($fname, $lname),
        Cvoid,
        (Ref{c_spmat{$elty}}, UInt8, $elty, Ptr{$elty}, $elty, Ptr{$elty}, Cint),
        spmat,
        transp,
        alpha,
        x,
    ...
    @inline qrm_spmat_mv!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      alpha::$elty,
      x::Vector{$elty},
      beta::$elty,
      y::Vector{$elty},
    ) = qrm_matmul(spmat.parent, alpha, x, beta, y, transp = 't')
    @inline qrm_spmat_mv!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      alpha::$elty,
      x::Matrix{$elty},
    ...
    @inline qrm_spmat_mv!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      alpha::$elty,
      x::Vector{$elty},
      beta::$elty,
      y::Vector{$elty},
    ) = qrm_matmul(spmat.parent, alpha, x, beta, y, transp = 'c')
    @inline qrm_spmat_mv!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      alpha::$elty,
      x::Matrix{$elty},
...
for (fname, lname, elty, subty) in (
  ("sqrm_spmat_nrm_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_spmat_nrm(spmat::qrm_spmat{$elty}; ntype::Char = 'f')
      nrm = Ref{$subty}(0)
      err =
        ccall(($fname, $lname), Cint, (Ref{c_spmat{$elty}}, UInt8, Ref{$subty}), spmat, ntype, nrm)
      (err ≠ 0) && throw(ErrorException(error_handling(err)))
      return nrm[]
    end
  end
...
for (fname, lname, elty, subty) in (
  ("sqrm_vecnrm_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_vecnrm(x::Vector{$elty}; ntype::Char = '2')
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ptr{$elty}, Cint, Cint, UInt8, Ref{$subty}),
        x,
        n,
        nrhs,
        ntype,
    ...
    function qrm_vecnrm(x::Matrix{$elty}; ntype::Char = '2')
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ptr{$elty}, Cint, Cint, UInt8, Ptr{$subty}),
        x,
        n,
        nrhs,
        ntype,
...
for (fname, lname, elty, subty) in (
  ("sqrm_spbackslash_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_spbackslash!(
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ptr{$elty}, Ptr{$elty}, Cint, UInt8),
        spmat,
        b,
        x,
        nrhs,
    ...
    @inline qrm_spbackslash!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_spbackslash!(spmat.parent, b, x, transp = 't')
    @inline qrm_spbackslash!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_spbackslash(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 't')
    @inline qrm_spbackslash(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 't')
    ...
    @inline qrm_spbackslash!(
      ...
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_spbackslash!(spmat.parent, b, x, transp = 'c')
    @inline qrm_spbackslash!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_spbackslash(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 'c')
    @inline qrm_spbackslash(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 'c')
    ...
    @inline (\)(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 't')
    @inline (\)(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 't')

    @inline (\)(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 'c')
    @inline (\)(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spmat.parent, b, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_spfct_backslash_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_spbackslash!(
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spfct{$elty}}, Ptr{$elty}, Ptr{$elty}, Cint, UInt8),
        spfct,
        b,
        x,
        nrhs,
    ...
    @inline qrm_spbackslash!(
      ...
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_spbackslash!(spfct.parent, b, x, transp = 't')
    @inline qrm_spbackslash!(
      spfct::Transpose{$elty, qrm_spfct{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_spbackslash(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 't')
    @inline qrm_spbackslash(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 't')
    ...
    @inline qrm_spbackslash!(
      ...
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_spbackslash!(spfct.parent, b, x, transp = 'c')
    @inline qrm_spbackslash!(
      spfct::Adjoint{$elty, qrm_spfct{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_spbackslash(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 'c')
    @inline qrm_spbackslash(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 'c')
    ...
    @inline (\)(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 't')
    @inline (\)(spfct::Transpose{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 't')

    @inline (\)(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Vector{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 'c')
    @inline (\)(spfct::Adjoint{$elty, qrm_spfct{$elty}}, b::Matrix{$elty}) =
      qrm_spbackslash(spfct.parent, b, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_spposv_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_spposv!(spmat::qrm_spmat{$elty}, b::Vector{$elty}, x::Vector{$elty})
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ptr{$elty}, Ptr{$elty}, Cint),
        spmat,
        b,
        x,
        nrhs,
...
for (fname, lname, elty, subty) in (
  ("sqrm_least_squares_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    ...
    @inline qrm_least_squares!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_least_squares!(spmat.parent, b, x, transp = 't')
    @inline qrm_least_squares!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_least_squares(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_least_squares(spmat.parent, b, transp = 't')
    @inline qrm_least_squares(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_least_squares(spmat.parent, b, transp = 't')
    ...
    @inline qrm_least_squares!(
      ...
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_least_squares!(spmat.parent, b, x, transp = 'c')
    @inline qrm_least_squares!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_least_squares(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_least_squares(spmat.parent, b, transp = 'c')
    @inline qrm_least_squares(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_least_squares(spmat.parent, b, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_min_norm_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    ...
    @inline qrm_min_norm!(
      ...
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_min_norm!(spmat.parent, b, x, transp = 't')
    @inline qrm_min_norm!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_min_norm(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_min_norm(spmat.parent, b, transp = 't')
    @inline qrm_min_norm(spmat::Transpose{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_min_norm(spmat.parent, b, transp = 't')
    ...
    @inline qrm_min_norm!(
      ...
      b::Vector{$elty},
      x::Vector{$elty},
    ) = qrm_min_norm!(spmat.parent, b, x, transp = 'c')
    @inline qrm_min_norm!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
    ...
    @inline qrm_min_norm(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Vector{$elty}) =
      qrm_min_norm(spmat.parent, b, transp = 'c')
    @inline qrm_min_norm(spmat::Adjoint{$elty, qrm_spmat{$elty}}, b::Matrix{$elty}) =
      qrm_min_norm(spmat.parent, b, transp = 'c')
  end
end

for (fname, lname, elty, subty) in (
  ("sqrm_residual_norm_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_residual_norm(
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ptr{$elty}, Ptr{$elty}, Cint, Ref{$subty}, UInt8),
        spmat,
        b,
        x,
        nrhs,
    ...
    function qrm_residual_norm(
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ptr{$elty}, Ptr{$elty}, Cint, Ptr{$subty}, UInt8),
        spmat,
        b,
        x,
        nrhs,
    ...
    @inline qrm_residual_norm!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
      nrm::Vector{$elty},
    ) = qrm_residual_norm!(spmat.parent, b, x, transp = 't')
    @inline qrm_residual_norm!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      b::Matrix{$elty},
      x::Matrix{$elty},
      nrm::Vector{$elty},
    ) = qrm_residual_norm!(spmat.parent, b, x, transp = 'c')
...
for (fname, lname, elty, subty) in (
  ("sqrm_residual_orth_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_residual_orth(spmat::qrm_spmat{$elty}, r::Vector{$elty}; transp::Char = 'n')
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ptr{$elty}, Cint, Ref{$subty}, UInt8),
        spmat,
        r,
        nrhs,
        nrm,
    ...
    function qrm_residual_orth(spmat::qrm_spmat{$elty}, r::Matrix{$elty}; transp::Char = 'n')
      ...
      err = ccall(
        ($fname, $lname),
        Cint,
        (Ref{c_spmat{$elty}}, Ptr{$elty}, Cint, Ptr{$subty}, UInt8),
        spmat,
        r,
        nrhs,
        nrm,
    ...
    @inline qrm_residual_orth!(
      spmat::Transpose{$elty, qrm_spmat{$elty}},
      r::Matrix{$elty},
      nrm::Vector{$elty},
    ) = qrm_residual_orth!(spmat.parent, r, transp = 't')
    @inline qrm_residual_orth!(
      spmat::Adjoint{$elty, qrm_spmat{$elty}},
      r::Matrix{$elty},
      nrm::Vector{$elty},
    ) = qrm_residual_orth!(spmat.parent, r, transp = 'c')

    @inline qrm_residual_orth(spmat::Transpose{$elty, qrm_spmat{$elty}}, r::Vector{$elty}) =
      qrm_residual_orth(spmat.parent, r, transp = 't')
    @inline qrm_residual_orth(spmat::Transpose{$elty, qrm_spmat{$elty}}, r::Matrix{$elty}) =
      qrm_residual_orth(spmat.parent, r, transp = 't')

    @inline qrm_residual_orth(spmat::Adjoint{$elty, qrm_spmat{$elty}}, r::Vector{$elty}) =
      qrm_residual_orth(spmat.parent, r, transp = 'c')
    @inline qrm_residual_orth(spmat::Adjoint{$elty, qrm_spmat{$elty}}, r::Matrix{$elty}) =
      qrm_residual_orth(spmat.parent, r, transp = 'c')
  end
end
...
for (finame, frname, lname, elty, subty) in (
  ("sqrm_spfct_set_i4_c", "sqrm_spfct_set_r4_c", libsqrm, Float32, Float32),
  ...
  @eval begin
    function qrm_set(spfct::qrm_spfct{$elty}, str::String, val::Number)
      ...
      elseif str ∈ RCNTL
        err =
          ccall(($frname, $lname), Cint, (Ref{c_spfct{$elty}}, Cstring, Cfloat), spfct, str, val)
      else
        err = Int32(23)
      end
      (err ≠ 0) && throw(ErrorException(error_handling(err)))
...
for (finame, frname, lname, elty, subty) in (
  ("sqrm_spfct_get_i8_c", "sqrm_spfct_get_r4_c", libsqrm, Float32, Float32),
    ...
    function qrm_get(spfct::qrm_spfct{$elty}, str::String)
      if (str ∈ PICNTL) || (str ∈ STATS)
        ...
        err = ccall(
          ($finame, $lname),
          Cint,
          (Ref{c_spfct{$elty}}, Cstring, Ref{Clonglong}),
          spfct,
          str,
          val,
        )
      elseif str ∈ RCNTL
        ...
        err = ccall(
          ($frname, $lname),
          Cint,
          (Ref{c_spfct{$elty}}, Cstring, Ref{Cfloat}),
          spfct,
          str,
          val,
        )
